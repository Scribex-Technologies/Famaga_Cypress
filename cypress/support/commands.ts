...
const CommandsElements = {
    ...
    credentialsError: '.mantine-InputWrapper-error.mantine-PasswordInput-error',
    emailError: '.mantine-InputWrapper-error.mantine-TextInput-error',
    cancelBtn: '.buildhero-12fdmdc > .buildhero-UnstyledButton-root > .buildhero-1wpc1xj',
    ...
    errorMsg: '#name-error',
    creationWindow: '.buildhero-Paper-root.buildhero-Modal-content.buildhero-Modal-content.buildhero-chy3l8',
    nameField: '#name',
...
Cypress.Commands.add('loginWithErrorHandling', (email: string, password: string) => {
    cy.intercept({
        method: 'POST',
        url: '/auth/email/login',
    }).as('loginRequest')
    cy.get(CommandsElements.email).type(email)
    cy.get(CommandsElements.password).type(password)
    cy.get(CommandsElements.signInBtn).click()
    cy.wait('@loginRequest').then((interception) => {
        if (interception && interception.response) {
            const statusCode = interception.response.statusCode
            if (statusCode === 400 || statusCode === 401) {
                cy.get(CommandsElements.credentialsError, { timeout: 10000 })
                    .should('be.visible')
                    .and('contain', generalTexts.invalidCredentialsMsg)
                    .then(() => {
                        cy.log('Login failed due to invalid credentials')
                    })
            } else {
                cy.log('Login successful')
            }
        } else {
            cy.log('Request did not complete or response is undefined.')
        }
    })
})
...
Cypress.Commands.add('verifyErrorMessage', (expectedMessage) => {
    cy.get(CommandsElements.imageUploadFileMsg).should('contain', expectedMessage)
})
