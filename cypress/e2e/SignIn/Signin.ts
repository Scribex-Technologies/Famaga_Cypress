...
When('I input valid Email address and invalid Password and click on the Sign in button', () => {
    cy.loginWithErrorHandling(generalTexts.mainEmail, 'invalidPassword')
})
...
When('I input invalid Email address and valid Password and click on the button Sign in', () => {
    signInPage.signIn('tatevikscal.io', generalTexts.mainPassword)
})
...
Then(
    'I see the user is not signed in and the validation messages displayed under the email and password fields',
    () => {
        cy.invalidEmailMsgIsDisplayed(generalTexts.emptyEmailMsg)
        cy.invalidCredentialsMsgIsDisplayed(generalTexts.emptyPasswordMsg)
    }
